...
Чтобы создать переменную нужно написать `(var id Expr)`. `id` - строка без кавычек, значение `Expr` вычисляется только в случае, если операции возвращают какое-то значение. Возвращаемое значение - `Expr.SIMPLE`.
