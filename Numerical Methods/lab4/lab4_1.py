...
def euler_method(f, g, y2, z0, interval, h):
    """
...
def runge_kutta_method(f, g, y2, z0, interval, h, return_z=False, exclude_num=None):
    """
    Solve cauchy problem:
    y' = g(x, y, z)
    z' = f(x, y, z)
...
def adams_method(f, g, y2, z0, interval, h):
    """
    Solve cauchy problem:
    y' = g(x, y, z)
    z' = f(x, y, z)
    ...
    Returns solution - tabular function y=f(x) as two arrays: x, y
    """
    x_runge, y_runge, z_runge = runge_kutta_method(f, g, y2, z0, interval, h, return_z=True)
    x = x_runge
    y = y_runge[:4]
    z = z_runge[:4]
...
if __name__ == '__main__':
    y2 = math.sqrt(2)  # y(0) = 1
    dy2 = math.sqrt(2) * 3/2  # y'(0) = 1
    interval = (2, 3)  # x in [0; 1]
    h = 0.1
    print_task()

    x_euler, y_euler = euler_method(f, g, y2, dy2, interval, h)
    plt.plot(x_euler, y_euler, label=f'euler method, step={h}')
    x_euler2, y_euler2 = euler_method(f, g, y2, dy2, interval, h/2)
    plt.plot(x_euler2, y_euler2, label=f'euler method, step={h/2}')

    x_runge, y_runge = runge_kutta_method(f, g, y2, dy2, interval, h)
    plt.plot(x_runge, y_runge, label=f'runge kutta method, step={h}')
    x_runge2, y_runge2 = runge_kutta_method(f, g, y2, dy2, interval, h/2)
    plt.plot(x_runge2, y_runge2, label=f'runge kutta method, step={h/2}')

    x_adams, y_adams = adams_method(f, g, y2, dy2, interval, h)
    plt.plot(x_adams, y_adams, label=f'adams method, step={h}')
    x_adams2, y_adams2 = adams_method(f, g, y2, dy2, interval, h/2)
