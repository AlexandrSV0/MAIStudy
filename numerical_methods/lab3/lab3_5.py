...
def integrate_by_trapeze(func_values, h):
    return h * (func_values[0]/2 + func_values[-1]/2 + sum([func_values[i] for i in range(1, len(func_values) - 1)]))
...
if __name__ == '__main__':
    ...
    print(f'* для метода прямоугольников = {runge_romberg_method(h1, h2, rectangle_result_h1, rectangle_result_h2, 3):.17}')
    print(f'* для метода трапеций        = {runge_romberg_method(h1, h2, trapeze_result_h1, trapeze_result_h2, 3):.17}')
    print(f'* для метода Симпсона        = {runge_romberg_method(h1, h2, simpson_result_h1, simpson_result_h2, 3):.17}')